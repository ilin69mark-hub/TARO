...
- [ ] S09 Docker/композ — GWT: Given контейнер, When whoami, Then app (не root); tzdata (MSK работает); HEALTHCHECK; compose без дефолтов (`:?...`); VAPID-приватник удален из git — связь: `Dockerfile*/compose`
- [ ] S10 Nginx/deploy — GWT: Given прод-конфиг, When проверяю, Then TLS+headers+SSE-буферинг off+лимиты diary/payments; cron с auth работает; deploy не маскирует ошибки — связь: `nginx/deploy`
